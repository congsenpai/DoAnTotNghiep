...
class TransactionInfo extends StatefulWidget {
  ...
  const TransactionInfo(
      {super.key, required this.name, required this.amount, required this.note, required this.user, required this.walletID});
...
class _TransactionInfoState extends State<TransactionInfo> {
  ...
  Future<void> onPayment() async {
    final paymentUrl = VNPAYFlutter.instance.generatePaymentUrl(
      ...
      vnpayHashKey: 'WSBCHHFZBEGYEQNOQHVKLNCGZVHQTHMU',
      //vnpay hash key, get from vnpay
      vnPayHashType: VNPayHashType
          .HMACSHA512,
      //hash type. Default is HMACSHA512, you can chang it in: https://sandbox.vnpayment.vn/merchantv2,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text(AppLocalizations.of(context).translate('VNPay'))),
      body: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            responseCode == "00"
                ? Infomation(color: Colors.green,
              icon: Icons.check,
              text: 'Successfully transaction',) // Thêm giá trị màu hợp lệ
                : responseCode == "200"
                ? Infomation(color: Colors.yellow,
              icon: Icons.closed_caption_off_outlined,
              text: 'Loading transaction',)
                : Infomation(color: Colors.red,
              icon: Icons.check,
            ...
            responseCode == '200'
              ...
              child: Text(
                  AppLocalizations.of(context).translate('Return Main Screen')),
            ) : SizedBox(height: 20)
...
class Infomation extends StatelessWidget {
  ...
  const Infomation(
      {super.key, required this.color, this.icon, required this.text});
    ...
    return Material(
      ...
      child: Container(
        width: MediaQuery
            .of(context)
            .size
            .width / 2, // Thay thế Get.width
        padding: const EdgeInsets.all(8),
        child: Column(
          children: [
            if (icon != null) Icon(icon, color: color),
            // Kiểm tra null trước khi hiển thị icon
            Text(
