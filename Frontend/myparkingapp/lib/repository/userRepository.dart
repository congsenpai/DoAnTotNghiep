...
class UserRepository {
  ...
  Future<APIResult> giveAllUserByPage_BySearch_ByOWNER_ROLE(int page, String token, String ROLE, String searchText) async {
    try {
      ...
      APIResult apiResult = APIResult(1,0, code: 200, message: "", result: user);
      return apiResult;
    } catch (e) {
      throw Exception("Lỗi kết nối: $e");
    }
  ...
  Future<APIResult> updatedUser(User user, String token) async{
    ...
    try {
      ...
      if(response.statusCode == 200){
        User user = User.fromJson(responseData["result"]);
        APIResult apiResult= APIResult(0,1, code: code, message: message, result: user);
        return apiResult;
      }
      else {
        APIResult apiResult= APIResult(0,1, code: code, message: message, result: "null");
        return apiResult;
      }
    }
    catch(e){
